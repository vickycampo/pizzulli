...
if ($action == "updateFolder") {
    ...
    $result['section'] = $cms_xml->updateSection($id, $new_name, $descr1, $descr2, $descr3, $descr4, $show_preview);
...
if ($action == "uploadThumb") {
    ...
    if (isset($_FILES['Filedata'])) {
        ...
        if (!$res) {
            ...
            $images = new Images($settings);
            $res = $images->replaceThumb($file, $name, $path);
...
if ($action == "addFile") {
    ...
    if (isset($_FILES['Filedata'])) {
        ...
        if (!$res) {
            ...
            $result['status'] = 'error';
            $result['error'] = 'addFile '.error_get_last();
		...
		if (!file_exists('tmp')) {
		    mkdir('tmp', 0755, true);
                ...
                $k = 0;
                while(file_exists('tmp/lock.lock') && $k<10){
                  $k++;
                  sleep(2);
                }
                $lock = fopen('tmp/lock.lock', 'a');
                $cms_xml = new CMS_XML();
